...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public static final String COL_ITEM3 = "Quantity";
    public static final String COL_ITEM4 = "UTD";
    public static final String COL_ITEM5 = "Berat";
    public static final String COL_ITEM6 = "WRIN";
    //end of table item
    ...
    public static final String COL_PO_RN_INV =  "";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("create table " + TABLE_ITEM + " (itemId INTEGER PRIMARY KEY AUTOINCREMENT,itemName TEXT," +
                "Quantity INTEGER,UTD TEXT,Berat INTEGER,WRIN TEXT)");
        ...
        db.execSQL("create table " + TABLE_INFO_TRUCK + " (truckId INTEGER PRIMARY KEY AUTOINCREMENT,Idtruckcondition INTEGER," +
                ...
                "FOREIGN KEY (Idseal) REFERENCES t_data (sealId))");
        ...
        db.execSQL("create table " + TABLE_QIPCRITERIA + " (criteriaId INTEGER PRIMARY KEY, Idjenisitem INTEGER," +
                ...
                "FOREIGN KEY (Idjenisitem) REFERENCES t_jenisitem(jenisitemId))");

        db.execSQL("create table " + TABLE_JENIS_ITEM + " (jenisitemId INTEGER PRIMARY KEY NOT NULL, jenisitem TEXT )");
    }
    ...
    public boolean insertDataItem(String itemName,Integer qty,Integer utd,Integer berat,String WRIN) {
        //ngebuka database
        ...
        contentValues.put(COL_ITEM3,qty);
        contentValues.put(COL_ITEM4,utd);
        contentValues.put(COL_ITEM5,berat);
        contentValues.put(COL_ITEM6,WRIN);
    ...
    public ItemSetGet getItemData(Integer itemId) {
        ...
        ist.setQuantity(c.getInt(c.getColumnIndex(COL_ITEM3)));
        ist.setUTD(c.getString(c.getColumnIndex(COL_ITEM4)));
        ist.setBerat(c.getInt(c.getColumnIndex(COL_ITEM5)));
        ist.setWRIN(c.getString(c.getColumnIndex(COL_ITEM6)));
    ...
    public List<SupplierSetGet> getAllSupplierData() {
        List<SupplierSetGet> supliersetget = new ArrayList<SupplierSetGet>();
        ...
        if (!selectQuery.isEmpty()) {
            if (c.moveToFirst()) {
                do {
                    SupplierSetGet ssg = new SupplierSetGet();
                    ssg.setSuplierName(c.getString(c.getColumnIndex(COL_SUPLIER2)));
                    ssg.setLocationItem(c.getString(c.getColumnIndex(COL_SUPLIER3)));
                    ssg.setIdItem(c.getInt(c.getColumnIndex(COL_SUPLIER4)));

                    // adding to item
                    supliersetget.add(ssg);
                } while (c.moveToNext());
            }

            return supliersetget;
        }else {
            return null;
        }
    }
